 ...
 */
    ...
    (() => {
        ...
        window.addEventListener("DOMContentLoaded", (event => {
            ...
            phoneMask.mask(document.querySelectorAll("._phone input"));
            let inputsName = document.querySelectorAll("._person input");
            inputsName.forEach((name => {
                name.addEventListener("keypress", (function(e) {
                    if (e.key === "." && this.value.includes(".")) e.preventDefault(); else if (!/^[a-zа-яё]+$/i.test(e.key)) e.preventDefault();
                }));
            }));
        }));
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                if (document.querySelector(".about__slider")) {
                    let aboutSlider = new swiper_core_Swiper(".about__slider", {
                    ...
                    let aboutThumbs = new swiper_core_Swiper(".about__thumbs", {
                        modules: [ Pagination, Controller, EffectCube ],
                        effect: "cube",
                        grabCursor: true,
                        cubeEffect: {
                            shadow: true,
                    ...
                    aboutSlider.controller.control = aboutThumbs;
                    aboutThumbs.controller.control = aboutSlider;
                }
                if (document.querySelector(".project__slider")) {
                    let projSliders = document.querySelectorAll(".project__slider");
                    projSliders.forEach((projSlider => {
                        new swiper_core_Swiper(projSlider, {
                            modules: [ Navigation ],
                            observer: true,
                            observeParents: true,
                            slidesPerView: 1,
                ...
                if (document.querySelector(".equip__slider")) {
                    let equipSlider = new swiper_core_Swiper(".equip__slider", {
                        ...
                        on: {
                            click(event) {
                                equipSlider.slideTo(this.clickedIndex);
                            }
                        },
                        breakpoints: {
                            320: {
...
***************************************************************************** */
        ...
        cards.forEach((card => {
            card.addEventListener("mouseover", (() => {
                for (let i = 0; i < cards.length; i++) cards[i].classList.remove("_active");
                card.classList.add("_active");
            }));
        }));
        window.addEventListener("DOMContentLoaded", (event => {
            for (let e of document.querySelectorAll("#square")) {
                e.style.setProperty("--value", e.value);
                e.style.setProperty("--min", e.min == "" ? "50" : e.min);
                e.style.setProperty("--max", e.max == "" ? "100" : e.max);
                e.addEventListener("input", (() => e.style.setProperty("--value", e.value)));
            }
        }));
        window.addEventListener("DOMContentLoaded", (event => {
            const roomType = document.querySelector(".calc__select");
            const repairType = Array.from(document.querySelectorAll(".repair-type"));
            const total = document.querySelector(".calc__sum");
            const time = document.querySelector("#square");
            roomType.addEventListener("change", (() => {
                currentSet.roomType = roomType.value;
                updatePrice();
            }));
            time.addEventListener("input", timeUpdate);
            repairType.forEach((el => {
            ...
            function optionUpdate(e) {
                currentSet.repairType = e.target.id;
                updatePrice();
            }
            ...
            let currentSet = {
                ...
                repairType: "cosmetic",
                getTariffPrice() {
                    return priceInfo.roomType[this.roomType];
                },
                getOptionPrice() {
                    return priceInfo.repairType[this.repairType];
                }
            };
        }));
        window.addEventListener("DOMContentLoaded", (event => {
            let quizPages = document.querySelectorAll(".quiz__info");
            for (let i = 0; i < quizPages.length; i++) {
                if (quizPages[i].querySelector(".quiz__btn")) quizPages[i].querySelector(".quiz__btn").addEventListener("click", (() => {
                    quizPages[i].classList.remove("_active");
                    quizPages[i + 1].classList.add("_active");
                }));
                if (quizPages[i].querySelector(".quiz__back")) quizPages[i].querySelector(".quiz__back").addEventListener("click", (() => {
                    quizPages[i].classList.remove("_active");
                    quizPages[i - 1].classList.add("_active");
                }));
            }
        ...
        if (!localStorage.getItem("cookie")) setTimeout((() => cookieAlert.classList.add("not-accepted")), 100);
        cookieAlertButton.addEventListener("click", (() => {
            localStorage.setItem("cookie", "accepted");
            cookieAlert.classList.remove("not-accepted");
        }));
        let toTopBtn = document.querySelector(".footer__to-top");
        function btnAppearing(btn) {
            document.addEventListener("scroll", (function() {
                if (document.querySelector("html").scrollTop > 600) btn.style.display = "flex"; else btn.style.display = "none";
            }));
        }
        btnAppearing(toTopBtn);
        ...
        window.addEventListener("scroll", (function() {
            ...
            if (scrollPosition > 600) header.classList.add("fixed"); else header.classList.remove("fixed");
        }));
        let menuItems = document.querySelectorAll(".menu__link");
        menuItems.forEach((menuItem => {
            menuItem.addEventListener("click", (() => {
                document.querySelector("html").classList.remove("menu-open", "lock");
            }));
        }));
        ...
        window.addEventListener("DOMContentLoaded", (event => {
            let mainForms = document.querySelectorAll("form");
            mainForms.forEach((mainForm => {
                mainForm.addEventListener("submit", (e => {
                    e.preventDefault();
                    grecaptcha.ready((function() {
                        grecaptcha.execute("6Lf5W_MqAAAAAKcAwwnIefxidYMR2zB-vEbi-Z5X", {
                            action: "contact"
                        }).then((function(token) {
                            var recaptchaResponse = mainForm.querySelector(`[name="recaptcha_response"]`);
                            ...
                            fetch("send.php", {
                                ...
                                mainForm.reset();
                                modules_flsModules.popup.open("#popup-success");
                                if (mainForm == document.querySelector("#form-quiz")) {
                                    let quizPages = document.querySelectorAll(".quiz__info");
                                    for (let i = 0; i < quizPages.length; i++) {
                                        quizPages[i].classList.remove("_active");
                                        if (i == 0) quizPages[i].classList.add("_active");
                                    }
                                }
                            })).catch((error => {
                                console.error(error);
                                modules_flsModules.popup.open("#popup-error");
                            }));
                        }));
                    }));
                }));
